...
if __name__ == '__main__':
    ...
    for i in range(len(files)):
        ...
        display_stat_n(best,stat_average,i,prob_cross,config.cross_over)
    ...
    f = open('./experiments/final_conflicts_'+config.cross_over+"_"+str(prob_cross)+'.csv', 'w')
    f.write('file,best,average\n')
    for i in range(len(best_solutions)):
        f.write(str(i+1)+","+str(best_solutions[i])+","+str(aver_solutions[i])+"\n")
    f.close()

    f2 = open('./experiments/generations_'+config.cross_over+"_"+str(prob_cross)+'.csv', 'w')
